...
namespace Fungus
{
    ...
    public class ObjectClicked : EventHandler
    {
        ...
        public void OnObjectClickedEvent(ObjectClickedEvent evt)
        {
        ...
        protected virtual IEnumerator DoExecuteBlock(int numFrames)
        {
            ...
            while (Vector2.Distance(clickableObject.transform.position, targetPos.transform.position) > clickableObject.DistanceToActivate)
            {
                if (!Clickable2D.isHovered && Input.GetMouseButtonDown(0))
                {
                    yield break;
                }
                yield return null;
